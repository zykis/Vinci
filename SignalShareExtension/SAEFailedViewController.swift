...
class SAEFailedViewController: UIViewController {
    ...
    override func loadView() {
        ...
        self.view.backgroundColor = UIColor.init(rgbHex: 0xF3F3F3)

        let logoImage = UIImage(named: "logoVinci4")
        let logoImageView = UIImageView(image: logoImage)
        logoImageView.contentMode = .scaleAspectFill
        self.view.addSubview(logoImageView)
//        logoImageView.autoCenterInSuperview()
        logoImageView.autoVCenterInSuperview()
...
//        logoImageView.autoSetDimension(.height, toSize: logoSize)
        logoImageView.autoPinEdge(toSuperviewEdge: .leading, withInset: -32.0)
        logoImageView.autoPinEdge(toSuperviewEdge: .trailing, withInset: -30.0)
