...
// VINCI extension
NSString *const TSGroupThreadInboxGroup = @"TSGroupThreadInboxGroup";
NSString *const TSGroupThreadArchiveGroup = @"TSGroupThreadArchiveGroup";
...
// VINCI extension
NSString *const TSCallMessageDatabaseViewExtensionName = @"TSCallMessageDatabaseViewExtensionName_Monotonic";
NSString *const TSAllCallsGroup = @"TSAllCallsGroup";
NSString *const TSMissedCallsGroup = @"TSMissedCallsGroup";
...
+ (void)asyncRegisterCallInteractionsDatabaseView:(OWSStorage *)storage
{
    YapDatabaseViewGrouping *viewGrouping = [YapDatabaseViewGrouping withObjectBlock:^NSString *(
                                                                                                 YapDatabaseReadTransaction *transaction, NSString *collection, NSString *key, id object) {
        ...
        if ( interaction.interactionType == OWSInteractionType_Call ) {
            TSCall* callMessage = (TSCall*)interaction;
            if ( callMessage.callType == RPRecentCallTypeIncomingMissed
                || callMessage.callType == RPRecentCallTypeIncomingMissedBecauseOfChangedIdentity ) {
    ...
    [self registerMessageDatabaseViewWithName:TSCallMessageDatabaseViewExtensionName
                                 viewGrouping:viewGrouping
                                      version:@"1"
                                      storage:storage];
}
...
+ (void)asyncRegisterThreadDatabaseView:(OWSStorage *)storage
{
    ...
    YapDatabaseViewGrouping *viewGrouping = [YapDatabaseViewGrouping withObjectBlock:^NSString *(
        ...
        if ( [object isKindOfClass:[TSContactThread class]] ) {
            return [thread isArchivedWithTransaction:transaction] ? TSArchiveGroup : TSInboxGroup;
        } else if ( [object isKindOfClass:[TSGroupThread class]] ) {
//            return [thread isArchivedWithTransaction:transaction] ? TSGroupThreadArchiveGroup : TSGroupThreadInboxGroup;
            return [thread isArchivedWithTransaction:transaction] ? TSArchiveGroup : TSGroupThreadInboxGroup;
        } else {
            return nil;
        }
    }];
